...
$(document).ready(function() {
    ...
    $('#sidebarChatMessage').keydown(function(event) {
        ...
        if (event.keyCode === 13) {
            ...
            var demoImage = $('.msg_container .avatar img').attr('src');

            // Create html
            var html = '';
            html += '	<div class="msg_container base_sent">';
            ...
            html += '	    </div>';
            html += '	  </div>';
            html += '	  <div class="col-md-3 col-xs-3 avatar">';
            html += '		 <img class="img-circle" src="' + demoImage + '" alt="">';
            html += '     </div>';
            html += '   </div>';
            var $new = $(html).hide();

            // Add to chat list
            $('.msg_container_base').prepend($new);
